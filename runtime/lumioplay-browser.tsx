...
function GamesGrid({
  ...
  /// Bumped when card-size settings change so the grid re-reads them.
  gridDensityTick: number
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars -- read to re-render on settings change
...
export function LumioplayBrowsePage(_props: BrowsePageProps) {
  ...
  // Re-render when the per-console card size changes in settings.
...
function rankPosterEntryForGame(entry: string, game: LumioplayGame): number {
  ...
  return (
    <div className="space-y-5">
      ...
      <GamesGrid
        gridDensityTick={gridDensityTick}
        games={filteredGames}
